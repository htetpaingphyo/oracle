...
FROM LIFE_DATA X 
WHERE POL_AUTHORIZED_DATE = (SELECT MAX(POL_AUTHORIZED_DATE) FROM LIFE_DATA) 
AND INT_INITIMATE_DT = (
    SELECT 
        CASE 
            WHEN MAX(INT_INITIMATE_DT) = '' THEN  MAX(INT_INITIMATE_DT) 
            ELSE MAX(INT_INITIMATE_DT) 
        END "INT_INITIMATE_DT" 
    FROM LIFE_DATA
);