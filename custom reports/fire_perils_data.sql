...
select 
    ...
    (select listagg(cus_name, ', ') within group (order by pol_policy_no)  
        ...
        and pol_seq_no=x.pol_seq_no) "CUS_NAME",  
    (select listagg(pol_transaction_type, ', ') within group (order by pol_policy_no) 
        from pol_data 
        where pol_policy_no=x.pol_policy_no 
        and pol_seq_no=x.pol_seq_no) "POLICY_TYPE", 
    ...
    ( select sum(ppr_premium) from pol_risk_perils where pol_policy_no=x.pol_policy_no and prs_name=x.prs_name group by prs_name) "RISK_PREMIUM", 
